...
const SendMessageForm = ({ socket, room }) => {
  ...
  const handleSendMessage = async () => {
    ...
    if (text.trim() !== "") {
      ...
      setText("");
        ...
        const response = await fetch('https://chatapp-task-api-ccc1e33bd7d6.herokuapp.com/api/messages/send', {
          method: 'POST',
          headers: {
  ...
  useEffect(() => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch('https://chatapp-task-api-ccc1e33bd7d6.herokuapp.com/api/messages', {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
